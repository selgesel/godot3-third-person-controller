...
func _begin_drag(event: InputEventScreenTouch):
    ...
    # joypad, and not where they pressed on the joypad
...
func _update_drag(event: InputEventScreenDrag):
    ...
    # calculate the raw drag vector and its raw length clamped to a max. drag distance, and divide
    ...
    # update the inner ring's current rotation and scale based on the value
...
func _end_drag(event: InputEventScreenTouch):
    # if the finger has been lifted off, stop tracking the finger and reset the value back to (0, 0)
    _touch_index = -1
    _is_dragging = false
    _value = Vector2.ZERO
...
func _process(delta):
    ...
    # lerp the ring's scale towards the current value so it grows and shrinks based on the drag strength
    ...
    # lerp the indicator's scale towards the current value so it grows and shrinks based on the drag strength
