...
export(float) var turn_speed: float = 3
export(float) var acceleration: float = 10
export(float) var cam_follow_speed: float = 6
export(float) var surge_cooldown: float = 1
export(float) var surge_charge_duration: float = .5
...
func physics_process(delta):
    ...
    # determine the blend position of the swimming animation blend space based on the player's current
    ...
    anim_pos = lerp(anim_pos, player.velocity.length() / 8.0, 4 * delta)
    player.anim_tree.set("parameters/Swimming/blend_position", anim_pos)

    # set the player's current horizontal movement based on the swim speed
    ...
    # determine the vertical swim angle and the corresponding vertical swim speed factor based on it
    ...
    if player.controls.is_swimming_up():
        ...
        # otherwise determine the vertical look angle of the player based on the camera's rotation
        vertical_angle = deg2rad(player.camera._rot_v)
